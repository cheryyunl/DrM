...
def get_ours(cfg_path, ckpt_path, **kwargs):
    ...
    new_payload = OrderedDict([
        ...
        for key, value in payload['r3m'].items()
        if not key.startswith("bc_")
    ])
    model.load_state_dict(new_payload, strict=False)
...
class Encoder(nn.Module):
    def __init__(self, obs_shape, encoder_cfg, encoder_ckpt, encoder_type, encoder_weights, freeze_encoder):
        super().__init__()

        assert len(obs_shape) == 3
        self.repr_dim = 32 * 35 * 35

        if encoder_weights == 'ours':
            self.convnet = get_ours(encoder_cfg, encoder_ckpt)
            self.repr_dim = 2048
...
class DrMAgent:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
                 ...
                 expectile, use_tb, state_dim, encoder_cfg, encoder_ckpt, 
                 encoder_type, encoder_weights, freeze_encoder):
        self.device = device
        ...
        # models
        self.encoder = Encoder(obs_shape, encoder_cfg, encoder_ckpt, 
                 encoder_type, encoder_weights, freeze_encoder).to(device)
